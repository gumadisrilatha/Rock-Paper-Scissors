...
function check(player, computer) {
  if (player == "rock" && computer == "rock") {
    index("Tie!", player, computer, "is-warning");
    score(false, false);
  } else if (player == "rock" && computer == "paper") {
    index("You Lose!", player, computer, "is-danger");
    score(false, true);
  } else if (player == "rock" && computer == "scissors") {
    index("You Win!", player, computer, "is-primary");
    score(true, false);
...
function index(note, player, computer, score) {
  document.getElementById(
    "note-if"
  ).innerHTML = `<div class="notification ${score} is-light" id="note-if">
    ...
    <p class="has-text-centered">
      <span id="state" class="has-text-weight-bold">${note}</span>
    </p>
  </div>`;
